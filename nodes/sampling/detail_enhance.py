...
from ..categories import SAMPLING
...
class UltimateDetailSamplerNode(io.ComfyNode):
    @classmethod
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="UltimateDetailSamplerNode",
            display_name="Detail Enhancer (Ultimate)",
            category=SAMPLING,
            inputs=[
                io.Sampler.Input("sampler"),
                io.Float.Input(
                    "enhance_factor", default=2.0, min=1.0, max=10.0, step=0.1,
                    tooltip="Time dilation in the selected sampling zone.",
                ),
                io.Float.Input(
                    "texture_boost", default=0.5, min=0.0, max=5.0, step=0.1,
                    tooltip="Sigma inflation for sharper texture. Values above 1.0 are strong.",
                ),
                io.Float.Input("start_percent", default=0.10, min=0.0, max=1.0, step=0.05),
                io.Float.Input("end_percent", default=0.80, min=0.0, max=1.0, step=0.05),
            ],
            outputs=[io.Sampler.Output(display_name="sampler")],
        )

    @classmethod
    def execute(cls, sampler, enhance_factor, texture_boost, start_percent, end_percent) -> io.NodeOutput:
        options = {
        ...
        return io.NodeOutput(KSAMPLER(enhance_sampler_wrapper, extra_options=options))


class UltimateDetailSigmasNode(io.ComfyNode):
    @classmethod
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="UltimateDetailSigmasNode",
            display_name="Detail Enhancer (Sigmas)",
            category=SAMPLING,
            inputs=[
    ...
    @classmethod
    def execute(cls, sigmas, enhance_factor, texture_boost, start_percent, end_percent) -> io.NodeOutput:
        result = process_ultimate_sigmas(
            sigmas,
